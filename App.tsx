...
const FunLoadingScreen: React.FC = () => {
    ...
    const content = [
        { icon: '📰', text: 'Scanning Headlines', sub: 'Reading between the lines...', color: 'bg-white', border: 'border-black' },
        { icon: '📺', text: 'Tuning Channels', sub: 'Filtering the static...', color: 'bg-g-blue', border: 'border-black' },
        { icon: '📡', text: 'Intercepting Feeds', sub: 'Decoding viral signals...', color: 'bg-g-red', border: 'border-black' }
    ];
...
const App: React.FC = () => {
  ...
  const startGame = async (difficulty: 'Easy' | 'Medium' | 'Hard') => {
    ...
    try {
      console.log('startGame: requesting items', { difficulty, count: 10 });
      const items = await generateQuizRound(difficulty, 10); // <-- correct order (difficulty, count)
      console.log('startGame: received', items?.length ?? 0, 'items');
      setQuizItems(items);
      setCurrentIndex(0);
    } catch (e) {
  ...
  const nextQuestion = useCallback(async () => {
     ...
     if (currentIndex < quizItems.length - 1) {
       ...
       setLoading(true);
       try {
         console.log('nextQuestion: requesting nextItems', { difficulty: gameState.difficulty, count: 5 });
         const nextItems = await generateQuizRound(gameState.difficulty, 5); // <-- correct order
         console.log('nextQuestion: received', nextItems?.length ?? 0, 'items');
    ...
    return (
      <div className="min-h-screen bg-g-yellow flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="bg-white dark:bg-zinc-900 max-w-md w-full p-8 border-4 border-black dark:border-white shadow-neo dark:shadow-neo-dark rounded-xl text-center relative z-10">
            <div className="w-20 h-20 bg-g-blue text-white text-3xl font-black flex items-center justify-center border-4 border-black mx-auto mb-6 transform rotate-3 shadow-[4px_4px_0px_0px_black]">SF</div>
            <h1 className="text-5xl font-display font-black text-black dark:text-white mb-2 uppercase tracking-tighter leading-none">Spot<br/>The<br/>Fake</h1>
    ...
    return (
        <div className={`min-h-screen ${isWin ? 'bg-g-green' : 'bg-g-red'} flex items-center justify-center p-4 relative`}>
            ...
            <div className="bg-white dark:bg-zinc-900 max-w-md w-full p-8 border-4 border-black dark:border-white shadow-neo-dark rounded-xl text-center">
                <h1 className="text-5xl font-display font-black text-black dark:text-white mb-2 uppercase">{isWin ? 'VICTORY!' : 'BUSTED!'}</h1>
                <p className="text-black dark:text-white font-bold mb-8">{isWin ? 'You survived the viral cycle.' : 'You believed the lies.'}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                     <button 
                        onClick={() => { playSound('CLICK'); setGameState(prev => ({...prev, status: 'IDLE'})); }}
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-hidden relative selection:bg-g-yellow selection:text-black bg-off-white dark:bg-neo-black">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 px-4 py-3 bg-white dark:bg-zinc-900 border-b-4 border-black dark:border-white flex justify-between items-center shadow-sm">
         ...
         <div className="flex items-center gap-6 font-mono">
             <div className="flex flex-col items-center">
                 ...
                 <span className={`font-black text-xl text-black dark:text-white ${gameState.streak > 2 ? 'text-g-red animate-bounce' : ''}`}>{gameState.streak}</span>
             </div>
             <div className="flex flex-col items-center">
         ...
         <div className="flex gap-2">
             <button onClick={() => setMusicEnabled(!musicEnabled)} className={`w-10 h-10 flex items-center justify-center border-2 border-black dark:border-white rounded-full transition-transform hover:scale-110 ${musicEnabled ? 'bg-g-green text-white' : 'bg-gray-200 dark:bg-zinc-700 text-gray-500'}`}>{musicEnabled ? ICONS.MUSIC : ICONS.MUSIC_OFF}</button>

             <button onClick={() => { playSound('CLICK'); setDarkMode(!darkMode); }} className="w-10 h-10 flex items-center justify-center border-2 border-black dark:border-white bg-g-yellow rounded-full hover:scale-110 transition-transform">{darkMode ? '☀️' : '🌙'}</button>
         </div>
      </header>
      ...
      <main className="flex-1 pt-28 pb-20 px-4 flex flex-col items-center justify-center relative w-full max-w-3xl mx-auto">
        ...
        {currentItem && (
            <div className="w-full flex flex-col items-center animate-slide-in">
                 ...
                 <GameCard key={currentItem.id} item={currentItem} onVote={handleVote} disabled={gameState.status === 'ANALYSIS'} />

                 <div className="w-full max-w-md mt-6 flex items-center gap-4">
                    <div className="flex-1">
                       <Timer duration={GAME_CONFIG.TIMER_SECONDS} onTimeUp={handleTimeUp} active={gameState.status === 'PLAYING'} />
                    </div>
      ...
      {showAnalysis && currentItem && lastGuess && (
        <AnalysisModal item={currentItem} userGuess={lastGuess} onClose={() => setShowAnalysis(false)} onNext={handleNextAfterAnalysis} />
      )}
    </div>
  );
};
