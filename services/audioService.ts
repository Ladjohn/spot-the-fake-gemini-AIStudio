...
/// NOTE: keep this file client-only. All TTS provider keys remain server-side in /api/tts.

///// SFX + MusicEngine (unchanged logic, kept client-side) /////
...
// --- Procedural Background Music (unchanged) ---
class MusicEngine {
...
/**
 * speakText(text):
 * - POSTs { text } to /api/tts
 * - expects { mime: string, data: base64 }
 * - decodes and plays (supports audio/wav|audio/mpeg|raw/pcm16)
 */
export const speakText = async (text: string) => {
  ...
  try {
    ...
    if (mime && mime.startsWith("audio/")) {
      // container formats (wav/mp3/ogg) — try decodeAudioData
      try {
        ...
        // fallback to raw PCM attempt below
        console.warn("decodeAudioData failed for container; will try raw PCM", err);
      }
    }

    // If provider returned raw PCM Int16 (common for some LLM TTS responses)
    // assume 24000Hz mono unless you know otherwise
    try {
...
function decodeRawPcm16(data: Uint8Array, ctx: AudioContext, sampleRate = 24000, numChannels = 1): AudioBuffer {
