...
const GameCard: React.FC<GameCardProps> = ({ item, onVote, disabled }) => {
  ...
  useEffect(() => {
    setImgSrc(item?.imageUrl ? item.imageUrl : "/placeholder.png");
  }, [item]);

  const handleImageError = () => {
    console.warn("Image failed:", item.imageUrl);
  ...
  return (
    <div className="w-full max-w-[90vw] md:max-w-md mx-auto my-4 relative select-none">
      ...
      >
        ...
        {/* CARD */}
        <div className="bg-white dark:bg-zinc-800 border-4 border-black dark:border-white shadow-neo dark:shadow-neo-dark rounded-xl overflow-hidden flex flex-col">
